    ...
    public static class FileManipulation
    {
        ...
        static void PrintFileProcessResult(FileProcessResult result, bool suppressGood = false)
        {
            ...
            else if (result.Status == "Exif")
            {
                if (!suppressGood)
                {
                    Console.Write($"{result.FileName,-30} ");
        ...
        public static void RenameFiles(List<FileProcessResult> allResults)
        {
            ...
            foreach (var result in allResults)
            {
                ...
                string newFileName = result.FileNameMod;
        ...
        /// This function works a little differently than the rest. 
        /// The loop will parse each file and undo the rename immediately. If there is nothing to do, it will continue the loop to the next file.
        /// </summary>
        public static void UndoRenameFiles(List<FileProcessResult> allResults, int intRenameSize, FileReadme readme)
        {
            if (allResults == null || allResults.Count == 0)
            {
                Console.WriteLine("No files to undo rename.");
                return;
            }
            ...
            foreach (var result in allResults)
            {
                ...
                // Remove prefix number and underscore (e.g., 001_, see also RENAME_SIZE)
                string fileNameNoPrefix = fileNameNoExt;
                int underscoreIdx = fileNameNoExt.IndexOf('_');
                // check if the prefix index is equal to RENAME_SIZE
                if (underscoreIdx != intRenameSize || underscoreIdx == -1)
                {
                    // if the underscore is not wehre we think it is, abort.
                    result.FileNameUndo = "";
                    result.Status = "Not possible to undo rename.";
                    continue;
                }
                
                if (underscoreIdx > 0 && int.TryParse(fileNameNoExt.Substring(0, underscoreIdx), out _))
                {
                    fileNameNoPrefix = fileNameNoExt.Substring(underscoreIdx + 1);
                }

                // Remove author from filename (if present)
                string author = readme.SearchFilterReturnName(fileNameNoPrefix);
                string fileNameNoAuthor = fileNameNoPrefix;
                if (!string.IsNullOrWhiteSpace(author) && fileNameNoPrefix.EndsWith("_" + author))
                {
                    fileNameNoAuthor = fileNameNoPrefix.Substring(0, fileNameNoPrefix.Length - (author.Length + 1));
                }

                string originalFileName = fileNameNoAuthor + extension;
                string originalFilePath = Path.Combine(directory, originalFileName);
                
                // Store the undo file name
                result.FileNameUndo = originalFileName;

                // if fileName matches originalFileName, abort.
                ...
                try
                {
                    ...
                    {
                        Console.WriteLine($"File already exists: {originalFilePath}. Skipping undo for {fileName}");
                        continue;
                    }

                    File.Move(result.FilePath, originalFilePath);
                    Console.WriteLine($"Reverted {fileName} to {originalFileName}");
                    result.FilePath = originalFilePath; // Update the file path in the result
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Error reverting {fileName}: {ex.Message}");
                }
            }
        }
    }
